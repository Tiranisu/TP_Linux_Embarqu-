...
sem_t FIN_CONCERT; // Création du sémaphore
int CPT = 0;
...
void *spectateur(void *arg){
    ...
    sem_wait(&FIN_CONCERT); // Attente de la libération du sémaphore et le met en état occupé
    sem_post(&FIN_CONCERT); // Libération du sémaphore
...
int main(){
    ...
    sem_init(&FIN_CONCERT, 0, 0); // Initialisation du sémaphore en état occupé par défaut
    ...
    sem_destroy(&DEB_CONCERT); // Destruction du sémaphore
    sem_destroy(&FIN_CONCERT); // Destruction du sémaphore
	return (0);
}
